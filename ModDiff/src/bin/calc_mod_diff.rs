...
impl CostFunction for BetaBinomialGLM {
    ...
    fn cost(&self, p: &Self::Param) -> std::result::Result<Self::Output, argmin::core::Error> {
        // FIX: Hard Bounds to prevent Nelder-Mead from exploring infinity
        for param in p {
            if *param < -15.0 || *param > 15.0 {
        ...
        for i in 0..self.ks.len() {
            ...
            pi = pi.clamp(1e-5, 1.0 - 1e-5); // Safe clamp
...
fn main() -> Result<()> {
    ...
    println!("Filtering windows by minimum depth (>= {}) and minimum CpGs (>= 5)...", min_depth);
    win_data.retain(|_, samples| {
        let mut g0_depth = 0.0; let mut g1_depth = 0.0;
        let mut max_cpgs = 0.0;
