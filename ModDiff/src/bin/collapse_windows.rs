...
fn main() -> Result<()> {
    ...
    let merge_sql = r#"
        ...
        WITH sig_wins AS (
            SELECT chrom, start, "end", diff_beta, p_value, adj_p_value, calculated_rho 
        ...
        SELECT 
            ...
            AVG(calculated_rho) AS mean_calculated_rho
        FROM groups 
        GROUP BY chrom, reg_id;
    "#;
    conn.execute(merge_sql, [])?;

    println!("Step 2: Saving collapsed DMRs to final table...");
    ...
    conn.execute(r#"
        ...
        SELECT 
            ...
            peak_adj_p_value,
            mean_calculated_rho AS calculated_rho
        FROM temp_merged
        ORDER BY adj_p_value ASC, ABS(diff_beta) DESC;
    "#, [])?;
    
    println!("DMRs collapsed successfully! Your mathematically conservative results are sorted in 'collapsed_dmrs'.");
    Ok(())
}